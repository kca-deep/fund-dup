...
| 한국어 NLP | 공백 기반 어절 분리 | **Kiwi (kiwipiepy)** | 고속 형태소 분석, pip 설치, 크로스플랫폼 |
...
| 한국어 NLP | Kiwi (kiwipiepy) | 고속 형태소 분석, 문장 분리, 크로스플랫폼 (pip only) |
...
| **한국어 NLP** | kiwipiepy | pip만으로 설치 완료, 외부 의존성 없음 |
...
│   │   │   ├── six_word_rule.py   # Phase 1: 6어절 규칙 (Kiwi)
...
| Kiwi 분석기 | `app.state.kiwi` | `kiwipiepy.Kiwi()` — 형태소 분석기 인스턴스 |
...
Kiwi의 내장 문장 분리 기능(`split_into_sents()`)을 활용한다. Kiwi는 종결어미 패턴을 자동으로 인식하여 문장을 분리하므로 별도의 규칙 구현이 불필요하다.
...
1. Kiwi POS 태깅으로 실질 형태소(명사/동사/형용사) 추출
...
1. Kiwi 형태소 분석으로 실질 형태소 추출 (명사, 동사, 형용사, 어근)
...
- **시스템 의존성:** `build-essential` (일부 Python 패키지 빌드용)
- **한국어 NLP:** kiwipiepy는 pip으로 설치되며 시스템 패키지 불필요
...
| Kiwi 모델 | pip 패키지에 포함 | ~50MB | pip install 시 자동 포함 |
...
**위험 요소:**
- Python 패키지 빌드 실패 → requirements.txt 사전 다운로드 준비
...
| Phase 1: 6어절 규칙 검사 (Kiwi + difflib) | analysis/six_word_rule.py |
...
- 한국어 형태소 분석 품질 → kiwipiepy 버전 고정 (모호성 해소 정확도 86.7%)
...
- **Kiwi (kiwipiepy)**: 고속 한국어 형태소 분석기, 크로스플랫폼 (Windows/Linux/macOS)
- **내장 사전**: 지속 업데이트, 모호성 해소 정확도 86.7%
- **문장 분리**: `split_into_sents()` 내장 기능으로 별도 구현 불필요
- **정보 밀도**: 한국어 1,000자 ≈ 영어 2,000~3,000자 → 청크 크기 1,000자가 적절
- **설치 간편성**: `pip install kiwipiepy` 한 줄, 시스템 패키지/Java 불필요
